...
public class StandardTriplanarParallaxInspector : ShaderGUI
{
    static class Styles
    {
        static public readonly GUIContent albedo = new GUIContent("Albedo", "Albedo (RGB)");
        static public readonly GUIContent top = new GUIContent("Top Albedo", "Albedo (RGB)");
    ...
    public override void OnGUI(MaterialEditor editor, MaterialProperty[] props)
    {
        ...
        editor.TexturePropertySingleLine(
            Styles.albedo, FindProperty("_TopTex", props), FindProperty("_Color", props)
        );
        editor.TexturePropertySingleLine(
            Styles.albedo, FindProperty("_SideTex", props)
        );
        editor.TexturePropertySingleLine(
            Styles.albedo, FindProperty("_BotTex", props)
        );

        editor.TexturePropertySingleLine(
            new GUIContent("Parallax Map"), FindProperty("_ParallaxMap", props)
        );

        editor.TexturePropertySingleLine(
            new GUIContent("Texture2DArray Blocks"), FindProperty("_Block2DArray", props)
        );
        editor.ShaderProperty(FindProperty("_SliceRange", props), "Texture Slice Index");
    ...
    {
        SetKeyword(material, "_PARALLAX_MAP", material.GetTexture("_ParallaxMap"));
